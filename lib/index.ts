...
export interface Field<T> {
  ...
  unique?: boolean;
  array?: boolean;
  default?: T extends StringConstructor
    ? string
...
export type AdditionalObjectFieldConstraintsCollection = {
  array?: boolean;
  [key: string]: any;
};
...
export interface FieldTypeContainer {
  ...
  object?: {
    [key: string]: FieldConstraintsCollection &
      AdditionalObjectFieldConstraintsCollection;
  };
}
...
// vallidation
export type ValidationCreator<T> = (
  message: string,
  validate: (toCheck: T, ...referenceVals: any[]) => boolean
) => any;
export type FinalValidationCreator<T> = (
  message: string,
